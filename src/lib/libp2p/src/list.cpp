...
void list::insert(const range6 &r) {
	this->_ranges6.push_back(r);
}

void list::insert(const list &l) {
	this->_ranges.insert(this->_ranges.end(), l._ranges.begin(), l._ranges.end());
	this->_ranges6.insert(this->_ranges6.end(), l._ranges6.begin(), l._ranges6.end());
}
...
class merge6_pred {
public:
	bool operator()(range6 &left, const range6 &right) const {
...
void list::optimize(bool aggressive) {
	...
	this->_ranges6.sort();
	this->_ranges6.unique(merge6_pred());
}
