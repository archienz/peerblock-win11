...
static wchar_t decode_utf8(InputIterator &iter, InputIterator last) {
	...
	if(iter == last) throw utf8_error("truncated utf-8");

	if(((*iter)&0x80) == 0) {
		ret=*iter++;
	}
	else if(((*iter)&0xE0) == 0xC0) {
		unsigned char c0 = (unsigned char)*iter++;
		if(iter == last) throw utf8_error("truncated utf-8");
		unsigned char c1 = (unsigned char)*iter++;
		if((c1 & 0xC0) != 0x80) throw utf8_error("invalid utf-8 continuation");
		ret = (wchar_t)(((c0 & 0x1F) << 6) | (c1 & 0x3F));
	}
		...
		unsigned char c1 = (unsigned char)*iter++;
		if(iter == last) throw utf8_error("truncated utf-8");
		unsigned char c2 = (unsigned char)*iter++;
		if((c1 & 0xC0) != 0x80 || (c2 & 0xC0) != 0x80) throw utf8_error("invalid utf-8 continuation");
		ret = (wchar_t)(((c0 & 0x0F) << 12) | ((c1 & 0x3F) << 6) | (c2 & 0x3F));
	}
