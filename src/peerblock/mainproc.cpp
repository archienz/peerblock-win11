			...
			try
			{
				...
				for(; !dates.empty(); dates.pop())
				{
					...
					if(fp) {
						...
						sqlite3_command cmd(g_con, "select time(time, 'localtime'),name,source,sourceport,destination,destport,protocol,action from t_history,t_names where time>=julianday(?, 'utc') and time<julianday(?, '+1 days', 'utc') and t_names.id=nameid;");
