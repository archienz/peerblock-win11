...
string ZipFile::GetCurrentFileName() {
	...
	int ret=unzGetCurrentFileInfo(this->fp, NULL, buf, sizeof(buf)-1, NULL, 0, NULL, 0);
...
pair<boost::shared_array<char>,size_t> ZipFile::ReadCurrentFile() {
	...
	const uLong kMaxUncompressed = 64UL * 1024UL * 1024UL;
	if(info.uncompressed_size == 0 || info.uncompressed_size > kMaxUncompressed)
		throw zip_error("unzReadCurrentFile", UNZ_BADZIPFILE);

	boost::shared_array<char> buf(new char[info.uncompressed_size]);

	ret=unzReadCurrentFile(fp, buf.get(), info.uncompressed_size);
	if(ret < 0 || (uLong)ret != info.uncompressed_size) throw zip_error("unzReadCurrentFile", ret);
