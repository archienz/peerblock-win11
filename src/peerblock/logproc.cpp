			...
			if(g_config.ShowAllowed || action.type == pbfilter::action::blocked || g_numlogged < 10)
			{
				...
				if(g_config.LogSize>0) {
					...
					switch(action.protocol) {
						...
						case IPPROTO_ICMPV6: lvi.pszText=_T("ICMPv6"); break;
#endif
									...
									try
									{
										...
										for(; !dates.empty(); dates.pop())
										{
											...
											if(fp) {
												...
												ostringstream sel;
												sel << "select time(time, 'localtime'),name,source,sourceport,destination,destport,protocol,action from t_history,t_names where time>=julianday(?, 'utc') and time<julianday(?, '+1 days', 'utc') and t_history.ROWID<" << midrowid << " and t_names.id=nameid;";
												sqlite3_command cmd(g_con, sel.str());
												cmd.bind(1, dates.front());
												cmd.bind(2, dates.front());
