...
static INT_PTR HistoryCalendar_OnNotify(HWND hwnd, int idCtrl, NMHDR *nmh) {
	if(nmh->code==MCN_SELECT && nmh->idFrom==IDC_CALENDAR) {
		...
		if(lock.is_locked()) {
			ostringstream dateos;
			dateos << ds->stStart;
			const string date=dateos.str();

			ostringstream ss;
			ss << "select distinct strftime('%m', time, 'localtime')-" << ds->stStart.wMonth << ",strftime('%d', time, 'localtime')-1 from t_history where action=1 and time >= julianday(?, 'utc') and time < julianday(?, '+3 months', 'utc');";
