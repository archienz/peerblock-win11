...
static pair<boost::shared_array<char>,size_t> UngzipFile(const path &file) {
	...
	vector<char> buf;
	const size_t kMaxUncompressed = 64u * 1024u * 1024u;
	...
	while((read=gzread(fp, tmp, sizeof(tmp)))) {
		...
		if(buf.size() + (size_t)read > kMaxUncompressed) {
			gzclose(fp);
			throw zip_error("gzread", Z_BUF_ERROR);
		}
	...
	switch(GetType(file))
	{
		...
		case File_7zip:
		{
			...
			for(unsigned int i=0; i<db.db.NumFiles; i++) {
				...
				if(f->IsDir) continue;
				if(f->Size > (64ull * 1024ull * 1024ull)) {
					SzArEx_Free(&db, &ai);
...
void LoadLists(HWND parent) {
	...
	for (vector<StaticList>::size_type i = 0; i < g_config.StaticLists.size(); ++i) {
		if (g_config.StaticLists[i].Enabled && g_config.StaticLists[i].Type == List::Allow)
			LoadList(g_config.StaticLists[i].File, allow);
	}
	for (vector<DynamicList>::size_type i = 0; i < g_config.DynamicLists.size(); ++i) {
		if (g_config.DynamicLists[i].Enabled && g_config.DynamicLists[i].Type == List::Allow)
			LoadList(g_config.DynamicLists[i].File(), allow);
	}
	...
	if(g_config.AllowLocal) {
		...
		set<p2p::ip6> locals6;
		GetLocalIps6(locals6, LOCALIP_ADAPTER|LOCALIP_DNS);
		for (set<p2p::ip6>::const_iterator iter = locals6.begin(); iter != locals6.end(); ++iter) {
			p2p::range6 r6(L"local IPv6");
			r6.start = r6.end = *iter;
			allow.insert(r6);
		}
		AddWellKnownLocalIPv6(allow);
	}
	...
	if(allow.size()>0) block.erase(allow);
	if(block.size()>0) block.optimize();
	if(allow.size6()>0) allow.optimize();
