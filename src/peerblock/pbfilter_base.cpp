...
void pbfilter_base::setranges(const p2p::list &ranges, bool block)
{
	...
	for(p2p::list::const_iterator6 iter = ranges.begin6(); iter != ranges.end6(); ++iter)
	{
		const wchar_t* &label = labels[iter->name];
		if(!label) {
			label = (const wchar_t*)labelsbuf.size();
	...
	if(ranges.size() > PB_MAX_RANGES)
		throw std::runtime_error("too many IP ranges");

	DWORD pbrsize = (DWORD)offsetof(PBRANGES, ranges[ranges.size()]);
	if(pbrsize < offsetof(PBRANGES, ranges[0]))
		throw std::runtime_error("range buffer overflow");
	...
	if(ret == ERROR_SUCCESS && ranges.size6() <= PB_MAX_RANGES) {
		DWORD pbr6size = (DWORD)offsetof(PBRANGES6, ranges[ranges.size6()]);
		if (pbr6size >= offsetof(PBRANGES6, ranges[0])) {
			PBRANGES6 *pbr6 = (PBRANGES6*)malloc(pbr6size ? pbr6size : offsetof(PBRANGES6, ranges[0]));
			if (pbr6) {
				pbr6->block = block ? 1 : 0;
				pbr6->count = (ULONG)ranges.size6();
				pbr6->labelsid = block ? m_blocklabelsid : m_allowlabelsid;
				unsigned int j = 0;
				for (p2p::list::const_iterator6 iter = ranges.begin6(); iter != ranges.end6(); ++iter) {
					pbr6->ranges[j].label = labels[iter->name];
					memcpy(pbr6->ranges[j].start, iter->start.b, 16);
					memcpy(pbr6->ranges[j].end, iter->end.b, 16);
					++j;
				}
				DWORD ret6 = m_filter.write(IOCTL_PEERBLOCK_SETRANGES6, pbr6,
					pbr6->count ? pbr6size : (DWORD)offsetof(PBRANGES6, ranges[0]));
				free(pbr6);
				if (ret6 == ERROR_SUCCESS && block)
					m_blockcount6 = (unsigned int)ranges.size6();
			}
		}
	}
...
void pbfilter_base::setflags(unsigned long flags)
{
	ULONG f = (ULONG)flags;
	DWORD ret = m_filter.write(IOCTL_PEERBLOCK_SETFLAGS, &f, sizeof(f));
	if (ret != ERROR_SUCCESS)
		throw win32_error("DeviceIoControl", ret);
}
...
void pbfilter_base::setdestinationports(const std::set<USHORT> &ports)
{
	...
	if (!ports.empty()) {
		nports = (USHORT*) malloc(sizeof(USHORT) * ports.size());
		if (!nports) throw std::bad_alloc();
	}
