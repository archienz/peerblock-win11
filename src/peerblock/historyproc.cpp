...
static string FormatHistoryDate(const SYSTEMTIME &st) {
	ostringstream os;
	os << st;
	return os.str();
}
...
static void History_LoadData(HWND hwnd, const SYSTEMTIME &st) {
	...
	ss << "time>=julianday(?, 'utc') and time<julianday(?, 'utc', '+1 day') order by t_history.ROWID desc;";

	const string date=FormatHistoryDate(st);
...
static void History_PerformSearch(HWND hwnd) {
	...
	if((info.iMask&HFM_FROM)) {
		...
		ss << " time>=julianday(?, 'utc')";
		fromDate=FormatHistoryDate(info.stFrom);
	}
	...
	if((info.iMask&HFM_TO)) {
		...
		ss << " time<julianday(?, 'utc', '+1 day')";
		toDate=FormatHistoryDate(info.stTo);
	}
	...
	try {
		...
		int bindIndex=1;
		if((info.iMask&HFM_FROM)) cmd.bind(bindIndex++, fromDate);
		if((info.iMask&HFM_TO)) cmd.bind(bindIndex++, toDate);
		if((info.iMask&HFM_RANGE)) cmd.bind(bindIndex++, name);
