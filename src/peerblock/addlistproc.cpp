	...
	switch(id) {
		...
		case IDOK:
		{
			...
			else
			{
				...
				if(!PbUrlIsHttps(url))
				{
					TRACEE("[addlistproc] [AddList_OnCommand]  * ERROR:  URL is not HTTPS!");
					MessageBox(hwnd, tstring(_T("List URLs must use HTTPS. http://, file://, and ftp:// are not allowed.")), IDS_INVALIDURL, MB_ICONERROR|MB_OK);
					break;
				}
