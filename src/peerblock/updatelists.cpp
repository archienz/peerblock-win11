...
private:
	...
	static int https_progress(void *ctx, double pct) {
		return progress_func(ctx, 100.0, pct, 0.0, 0.0);
	}
...
public:
	...
	int _Process()
	{
		...
		if(total>0)
		{
			...
			if(g_config.UpdateLists && updatelists)
			{
				...
				for(vector<DynamicList>::size_type i=0; i<g_config.DynamicLists.size(); i++)
				{
					...
					if(elapsedTime >= 43200 || !fileExists || fileSize==0)
					{
						...
						{
							hdata.push_back(data);
							TRACEV("[UpdateThread] [_Process]    queued dynamic-list download");
						}
						else
						{
			...
			{
				///////////////////////////////////////////////
				/// Perform Updates (HTTPS via WinHTTP)

				TRACEI("[UpdateThread] [_Process]    performing updates");
				for (vector<HandleData*>::size_type hi = 0; hi < hdata.size() && !aborted; ++hi)
				{
							HandleData *data = hdata[hi];

							tstring wurl = UTF8_TSTRING(data->url);
							time_t ims = 0;
							if (data->list && data->list->LastDownload && path::exists(data->list->File()))
								ims = data->list->LastDownload;

							PbHttpResult httpr = PbHttpsGet(
								wurl,
								data->fp,
								data->list ? NULL : &build,
								ims,
								g_agent,
								g_config.UpdateProxy,
								g_config.UpdateProxyType != CURLPROXY_HTTP,
								preconnect_func,
								&allowed,
								https_progress,
								data,
							...
							strncpy_s(data->errbuf, httpr.err, _TRUNCATE);
								...
								if(httpr.transport_ok)
								{
									TRACEV("[UpdateThread] [_Process]    HTTPS transport OK");
									long code = httpr.status;
									TCHAR chBuf[256];
								...
								{
									tstring strBuf = boost::str(tformat(_T("[UpdateThread] [_Process]    *  failed update, HTTPS error:[%1%]")) % httpr.winerr );
									TRACEBUFE(strBuf);
									data->list->FailedUpdate=true;
									...
									if(list) {
										...
										{
											tstring strBuf = boost::str(tformat(
												_T("[UpdateThread] [_Process]    ERROR CONTACTING URL, err:[%1%]"))
												% data->errbuf );
											TRACEBUFE(strBuf);
										...
										{
											tstring strBuf = boost::str(tformat(_T("[UpdateThread] [_Process]    ERROR CONTACTING URL, result:[%1%]")) % httpr.winerr );
											TRACEBUFE(strBuf);
											str=boost::str(tformat(LoadString(IDS_ERRORCONTACTINGWHY))%httpr.winerr);
										}
