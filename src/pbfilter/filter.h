...
// Hard caps to prevent integer overflow / nonpaged-pool exhaustion via IOCTL.
#define PB_MAX_RANGES           1000000UL
#define PB_MAX_PORTS            65535UL
#define PB_DEVICE_SDDL          L"D:P(A;;GA;;;SY)(A;;GA;;;BA)"
...
#define IOCTL_PEERBLOCK_SETRANGES6			CTL_CODE_PEERBLOCK(6, FILE_WRITE_DATA)
...
typedef struct TAG_PBIP6RANGE {
	LPCWSTR label;
	unsigned char start[16];
	unsigned char end[16];
} PBIP6RANGE;

typedef struct TAG_PBRANGES6 {
	ULONG block;
	ULONG labelsid;
	ULONG count;
	PBIP6RANGE ranges[1];
